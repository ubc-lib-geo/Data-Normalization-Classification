...
class GetData(object):
    ...
    def __init__(self,start_date='2000-01-01'):
        ...
        self.CA_PoliceKillings = pd.read_csv('Inputs/PID_Canada.csv',
                                              parse_dates=['date'],
                                              index_col=['id_victim']
                                             )
        ...
        self.CA_PoliceKillings=self.CA_PoliceKillings.loc[self.CA_PoliceKillings.index>=start_date]
        self.CA_PoliceKillings['armed_type'] =  self.CA_PoliceKillings['armed_type'].replace({
        ...
        self.CA_PoliceKillings['department'] =  self.CA_PoliceKillings['department'].replace({
        ...
        self.US_PoliceKillings=self.US_PoliceKillings.loc[self.US_PoliceKillings.index>=start_date]
        ...
        self.CA = CA_Provinces.join(self.CA_PoliceKillings.groupby('prov').count()["department"])
        self.CA = self.CA.rename(columns={'department':'Total_Killings'})
    ...
    # Quantiles
        ...
        start = 0
        end = math.ceil(max(self.US[column].max(),self.CA[column].max())*10)/10+.1
